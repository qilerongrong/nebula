;V.registPlugin("v.component.layout.layout5.header",function(){
	V.Classes.create({
		className:"v.component.layout.layout5.Header",
		superClass:"v.component.pages.Header",
		init:function(){
		    this.ns = "v.component.layout.layout5.header";
			this.resource = {
			    html:'template.html'
			}
		}
	...
	(function(Header){
		...
		Header.prototype.initEvent = function(){
            ...
            $('.theme_default',this.template).click(function(){
                that.theme('default');
            });
            $('.theme_red',this.template).click(function(){
                that.theme('red');
            });
			$('.logout',this.template).click(function(){
            	that.logout();
            })
			...
			$(".passwordChange",this.template).click(function(){
                ...
                dlg.setBtnsBar({btns:[
                    {text:that.getLang("BIN_CONFIRM"),style:"btn-primary",handler:function(){
                          ...
                          that.passwordSave();
                          dlg.close();
                    }},{text:that.getLang("BIN_CLOSE"),handler:dlg.close}
                ]});
                dlg.init({width:560,height:280,title:that.getLang("BIN_CGE_PASSWORD"),icons:['close'],isFull:true});
                var url = that.getPath()+"/assets/customPasswordSetting.html";
                $.ajax({
                    url:url,
                    dataType:'html',
            ...
            //跳转到菜单
            $('.userName',this.template).click(function(){
            	/**
            	 * menuCode:3级菜单;navCode:1级菜单
            	 * menuName:菜单名字
            	 * module:module
            	 */
            	var menuData = {menu:{plugin:'v.views.backoffice.authority.userList',menuCode:'10080203',navCode:'10080000',menuName:'系统用户与权限管理',module:'backoffice/authority/user'}};
            	V.MessageBus.publish({eventId:'loadPage',data:menuData});
            })
...
//                	   });
		...
		Header.prototype.initTime = function(){
			var date = new Date();
			var date_str = V.Util.formatDate(date,"YYYY年MM月DD日 hh:mm:ss");
			var weekArr = ["一","二","三","四","五","六","日"];
			date_str += " 星期"+weekArr[date.getDay()];
			$('.time',this.template).text(date_str);
      ...
      //       }
        ...
        Header.prototype.theme = function(theme){
            $.cookie('theme',theme);
            location.reload();
        }
...
//			})
        ...
        Header.prototype.logout = function(){
        	V.confirm("确定退出登录？",function(){
        	    window.location.href="j_spring_security_logout";
        	},null,true)
        }
        // Header.prototype.downloadLogo = function(){退出
        // 	var imgObj = $('#logo');
        ...
        // 	})
        // }
	})(V.Classes['v.component.layout.layout5.Header']);
},{plugins:['v.ui.alert','v.ui.confirm','v.ui.messageBox',"v.fn.validator"]});