;V.registPlugin("v.component.layout.layout5.navigation",function(){
	V.Classes.create({
		className:"v.component.layout.layout5.Navigation",
		superClass:"v.component.pages.Navigation",
		init:function(){
		    this.ns = "v.component.layout.layout5.navigation";
            this.maxSize = 5;
		}
	});
	(function(Nav){
		Nav.prototype.initEvent = function(){
			...
			//this.initBrand();
            var dashboard = $('<li class="dropdown active">\
            ...
            $('.nav',this.template).prepend('<li class="separator"></li>');
		...
		Nav.prototype.initNav = function(){
            ...
            $.each(menus,function(index){
            	if(index == that.maxSize && menus.length>that.maxSize){
                	that.initMore();
                	return false;
                }
                ...
                if(index < menus.length-1){
                    $('.nav',that.template).append(separator);
                }
            });
        ...
        Nav.prototype.initMore = function(){
        	...
        	var that = this;
        	$.each(this.menus.slice(that.maxSize),function(){
                var navCode = this.id;
	...
	})(V.Classes['v.component.layout.layout5.Navigation']);
