;V.registPlugin("v.views.resource.resourceList",function(){
	V.Classes.create({
		className:"v.views.resource.ResourceList",
		superClass:"v.views.commonDocket.CommonDocketList",
		init:function(){
            this.ns = 'v.views.resource.resourceList';
			this.module = '';
	...
	(function(List){
		...
		List.prototype.initList = function(){
			...
			list.setActionColumn({
				...
				render: function(record){
					...
					if (record.auditStatus == null || record.auditStatus == '' || record.auditStatus == CONSTANT.B2B.PROJECT_DOCKET_STATUS.STATUS_DRAFT) {
			...
			list.init({
				url: this.module+'/resource!init.action',
				hasData : true,
				sortable:false,
				checkable:false
			});
		...
		List.prototype.editDetail = function(record){
			...
			this.forward('v.views.resource.resourceEdit',options,function(inst){
					inst.addCrumb();
		...
		List.prototype.viewDetail = function(record) {
			...
			options.id=record.id;
			options.hasDetail = this.list.hasDetail;
			this.forward('v.views.resource.resourceView',options,function(inst){
				inst.addCrumb();
			});
		}
		...
		List.prototype.remove = function(record){
			...
			V.confirm(info,function ok(e){
				var url = that.module+'/resource!delete.action';
	            $.ajax({
	            	url:url,
	               	type:'POST',
	...
	})(V.Classes['v.views.resource.ResourceList']);
