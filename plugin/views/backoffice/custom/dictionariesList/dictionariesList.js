;V.registPlugin("v.views.backoffice.custom.dictionariesList",function() {
	V.Classes.create({
				className : "v.views.backoffice.custom.DictionariesList",
				superClass : "v.views.component.SearchList",
				init : function() {
					this.ns = 'v.views.backoffice.custom.dictionariesList';
					this.module = '';
	...
	(function(PartyRegistList) {
		...
		PartyRegistList.prototype.editTicket = function(record){
			...
			this.forward('v.views.backoffice.custom.dictionaries',options,function(p){
				p.addCrumb();
			});
		}
		PartyRegistList.prototype.addCrumb = function(){
			V.MessageBus.publish({eventId:"addCrumb",data:{ns:this.ns,options:this.options,name:'数据字典'}});
		}
	})(V.Classes['v.views.backoffice.custom.DictionariesList']);
}, {
