;V.registPlugin("v.views.tools.backoffice.payInterfaceLog",function(){
	V.Classes.create({
		className:"v.views.tools.backoffice.PayInterfaceLog",
		superClass:"v.views.commonDocket.CommonDocketList",
		init:function(){
            this.ns = 'v.views.tools.backoffice.payInterfaceLog';
			this.module = '';
		...
		List.prototype.initList = function(){
			...
			list.setActionColumn({
				...
				render: function(record){
					...
					html = $('<div class="action"><a class="view" href="javascript:void(0);" title="查看详情"><i class="icon-search"></i></a><div>');
...
//		    	tools.push({eventId:'add',text:"新增",icon:'icon-plus'});
			...
			list.init({
				url: this.module+'/pay-interface-log!init.action',
				hasData : true,
				sortable:false,
				checkable:false
			});
...
//				return;
		...
		List.prototype.viewDetail = function(record) {
			...
			var item3 = {
				name : 'salesOrderDocket',
				label : '订单号',
				value : record['salesOrderDocket']||'',
				type : V.Classes['v.component.Form'].TYPE.READONLY
			};
			var item4 = {
					name : 'amt',
...
//	                		that.list.removeRecord(record);
	...
	})(V.Classes['v.views.tools.backoffice.PayInterfaceLog']);
