;V.registPlugin("v.module.authority.authoritySetting",function(){
	V.Classes.create({
		className:"v.module.authority.AuthoritySetting",
		superClass:"v.Plugin",
		init: function(){
			this.config = null,
			this.data = {
			...
			this.ns = 'v.module.authority.authoritySetting';
			this.resource = {
				html1:'template.html'
			}
			this.EVENT = {
...
//            }
			...
			grid.init({
				checkable:false,
				container:container,
				url:that.data.module+'/role!menuList.action',
				columns:columns
			});
		}
...
//            columns.push({displayName:'类型',key:'homeType',width:320});
//			grid.init({
//				container:container,
...
//            }
            ...
            grid.init({
                container:container,
                url:that.data.module+'/role!listReport.action?roleId='+that.data.roleId,
                columns:columns
            });
		}
		...
		Authority.prototype.addMenu = function(){
            ...
            $.ajax({
                url:that.data.module+'/role!menuTree.action',
                type:'post',
                data:{
                	id:that.data.id,
                	roleId:that.data.roleId
...
//        }
        Authority.prototype.addReport = function(){
            ...
            list.init({
                ...
                checkable:true,
                url:that.data.module+'/role!listRepotAuth.action',
                columns:[
            ...
            addDlg.setBtnsBar({btns:[{text:"授权",style:"btn-primary",handler:function(){
                ...
                $.ajax({
                    url:that.data.module+'/role!saveReportAuth.action',
                    type:'post',
                    data: {
                    	id:that.data.id,
                    	roleId:that.data['roleId'],
...
//            })
//        }
	})(V.Classes['v.module.authority.AuthoritySetting']);
